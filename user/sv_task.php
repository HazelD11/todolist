...
if($act == "set_done") {
    ...
    if($task_team == 0){
        ...
        $sql = "SELECT t.*,cb.team AS cbt, cb.task_id AS cbtid, cb.team_name FROM task t
        JOIN collaborator cb ON cb.task_id = t.id 
        WHERE t.id='$id' AND t.user_id=$user_id AND t.team=$task_team";
        
        $sql2 = "SELECT cb.user_id AS cbuid FROM task t
        JOIN collaborator cb ON cb.task_id = t.id 
        WHERE t.id='$id' AND t.user_id=$user_id AND t.team=$task_team";
        $query2 = mysqli_query($conn, $sql2); 
        while($result2 = mysqli_fetch_array($query2)){
            $collaborator[] = $result2['cbuid'];
        }
    }
    ...
    echo "|".$task_id."|".$task_name."|".$task_date."|".$task_desc."|".$priority_id."|".$user_id."|".$category_id."|".$task_time."|".$status_id."|".$task_team."|".$team_name."|".json_encode($collaborator)."|";
    // echo implode("|",$collaborator)."|";

    // foreach($result2 AS $res){
    //     // echo implode("|",$collaborator)."|";
    //     $coll = $collaborator[$res];
    //     echo $coll."|";
    ...
    mysqli_query($conn, $sql);
        ...
        $sql2 = "UPDATE collaborator SET team_name = '$team_name' WHERE task_id = '$task_id'";
    ...
    if($collaborator == ""){
        $solo = 0;
        $sql = "INSERT INTO task(id,task_name,task_date,task_desc,priority_id,user_id,category_id,task_time,status_id,team) VALUES ('$task_id','$task_name','$task_date','$task_desc','$priority_id','$user_id','$category_id','$task_time','$status_id','$solo')";
        $query = mysqli_query($conn, $sql);
        
    }else if($collaborator != "" && $team_name != ""){
        $sql2 = "INSERT INTO task(id,task_name,task_date,task_desc,priority_id,user_id,category_id,task_time,status_id,team) VALUES ('$task_id','$task_name','$task_date','$task_desc','$priority_id','$user_id','$category_id','$task_time','$status_id','$team')";
        $query2 = mysqli_query($conn, $sql2);
        // $last_id = "SELECT LAST_INSERT_ID()" ; //task last insert id value
        $last_id = mysqli_insert_id($conn); //task last insert id value
        // $query_last_id = mysqli_query($conn, $last_id);
        // $result = mysqli_fetch_assoc($query_last_id);
        // echo $result;
        $owner = "owner";
        $co_owner = "co_owner";
        for($x=0;$x<count($collaborator);$x++){
            $collab = $collaborator[$counter];
            $sql3 = "INSERT INTO collaborator(id,task_id,user_id,team,team_name,collab_role) VALUES ('','$last_id','$collab','$team','$team_name','$co_owner')";
            $query3 = mysqli_query($conn, $sql3);
            $counter++;
        }
        $sql4 = "INSERT INTO collaborator(id,task_id,user_id,team,team_name,collab_role) VALUES ('','$last_id','$user_id','$team','$team_name','$owner')";
    ...
    $sql = "SELECT t.*, c.category_name, c.category_img FROM task t LEFT JOIN category c ON t.category_id = c.id WHERE user_id='$user_id' AND status_id=1 AND t.team=0 ORDER BY t.task_date DESC";
    $query = mysqli_query($conn, $sql);
    ...
    while ($result = mysqli_fetch_array($query)) {
        ...
        }else if($task_date == $current_date && $task_time < $current_time){
            $sql1 = "UPDATE task SET status_id=3 WHERE id='$task_id'";
            $query1 = mysqli_query($conn, $sql1);
        }else{
    ...
    // $counter = 0;
    // $status_id = 1;
    $team = $_POST['collab_team'];
    if($team != "title_team"){
        $sql = "SELECT t.*,cb.id,cb.task_id AS ctid,cb.user_id AS cbuid,cb.team AS cbt,cb.team_name,cb.collab_role,c.category_name, c.category_img FROM task t
            JOIN category c ON t.category_id = c.id
            JOIN collaborator cb on t.id = cb.task_id 
            WHERE cb.team=$team AND t.status_id=1 AND cb.user_id=$user_id ORDER BY t.task_date DESC
            ";
            $query = mysqli_query($conn, $sql);
    
    }else if($team == "title_team"){
        $sql = "SELECT t.*,cb.id,cb.task_id AS ctid,cb.user_id AS cbuid,cb.team AS cbt,cb.team_name,cb.collab_role,c.category_name, c.category_img FROM task t
            JOIN category c ON t.category_id = c.id
            JOIN collaborator cb on t.id = cb.task_id 
            WHERE t.status_id=1 AND cb.user_id=$user_id ORDER BY t.task_date DESC
            ";
            $query = mysqli_query($conn, $sql);
    }
    $check_none = mysqli_num_rows($query);
    ...
    while ($result = mysqli_fetch_array($query)) {
        ...
        $task_time = $result['task_time'];
        $team_name = $result['team_name'];
        $collab_role = $result['collab_role'];
        $status_id = $result['status_id'];
        $category = $result['category_name'];   
        $category_img = $result['category_img'];
        $current_date = date('Y-m-d');
        ...
        <div class="row" style="border :1px solid #FEFEFE;border-radius: 20px;">
            ...
            <?php 
            if($collab_role == "owner"){
            ?>
                <div class="col-2" style="margin-top:25px;">
                    <input type="button" name="delete_task" class="logout" id="delete_task<?php echo $task_id; ?>" value="Delete" onclick="delete_task(<?php echo $task_id;?>,<?php echo $task_team;?>)">
                </div>
                
                <div class="col-2" style="margin-top:25px;">
                    <input type="button" name="edit_task" class="logout" id="edit_task<?php echo $task_id; ?>" value="Edit" onclick="edit_task(<?php echo $task_id;?>,<?php echo $task_team;?>)">
                </div>
    ...
    $sql = "SELECT t.*, c.category_name, c.category_img FROM task t 
            JOIN category c ON t.category_id = c.id 
            WHERE t.user_id='$user_id' AND t.status_id=2 ORDER BY t.task_date DESC";
    $query = mysqli_query($conn, $sql);
    $check_none = mysqli_num_rows($query);
    if($check_none == 0){
        ?>
    ...
    if($task_team != 0){
        ...
        $query = mysqli_query($conn, $sql);
        $sql2 = "DELETE FROM collaborator WHERE task_id='$id'";
        $query2 = mysqli_query($conn, $sql2);
    }else{
        $sql = "DELETE FROM task WHERE id='$id'";
        $query = mysqli_query($conn, $sql);
...
$query = mysqli_query($conn,$sql);
    ...
    $sql = "SELECT t.*, rm.id AS rm_id,rm.reminder_date,rm.reminder_time,rm.task_id,rm.ringtone_id,rt.*,c.category_img FROM task t 
            ...
            JOIN category c ON t.category_id = c.id
            WHERE user_id='$user_id' AND rm.task_id = t.id AND t.status_id=1 AND (rm.reminder_date >= CURDATE() AND rm.reminder_time >= CURRENT_TIME())";
    $query = mysqli_query($conn, $sql);
    $check_none = mysqli_num_rows($query);
    if($check_none == 0){
        ?>
